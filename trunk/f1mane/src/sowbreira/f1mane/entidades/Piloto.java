...
public class Piloto implements Serializable {
	...
	private int calcularNovoIndex(InterfaceJogo controleJogo) {
		...
		ganho = processaSaidaDaPista(controleJogo, ganho);
	...
	public double processaSaidaDaPista(InterfaceJogo controleJogo, double ganho) {
		double valComp = 50;
		if (isJogadorHumano()) {
	...
	public boolean derrapa(InterfaceJogo controleJogo) {
		...
		mudarTracado(controleJogo.obterLadoDerrapa(pontoDerrapada),
				controleJogo, true);
		if (getPosicao() < 10)
	...
	public Rectangle2D centralizaDianteiraTrazeiraCarro(
			InterfaceJogo controleJogo) {
		return centralizaDianteiraTrazeiraCarro(controleJogo, null);
	}

	public Rectangle2D centralizaDianteiraTrazeiraCarro(
			InterfaceJogo controleJogo, Point p) {
		No noAtual = getNoAtual();
		boolean noPainel = false;
		if (p == null)
			p = noAtual.getPoint();
		else {
			noPainel = true;
		...
		Rectangle2D rectangle = new Rectangle2D.Double((p.x - Carro.MEIA_ALTURA
				* Carro.FATOR_AREA_CARRO), (p.y - Carro.MEIA_ALTURA
	...
	public void decStress(int val) {
		if (stress > 0 && (stress - val) > 0
				&& (Math.random() > ((920.0 - getPosicao() * 20) / 1000.0))) {
			stress -= val;
		}
	}
	...
	public boolean mudarTracado(int pos, InterfaceJogo interfaceJogo,
		...
		if (getTracadoAntigo() == 4 && pos != 4) {
			pos = 2;
		}
		if (getTracadoAntigo() == 5 && pos != 5) {
			pos = 1;
		}
