...
public class Piloto implements Serializable {
	...
	private void processaGanho(InterfaceJogo controleJogo) {
		...
		if (calculaDiffParaProximo > 25
				...
				&& (No.CURVA_ALTA.equals(getNoAtual().getTipo()) || No.CURVA_BAIXA
						.equals(getNoAtual().getTipo()))) {
			Carro frente = controleJogo.obterCarroNaFrente(this);
			if (frente != null && frente.getPiloto().verificaDesconcentrado()) {
				return;
			}
	...
	private void processaIAnovoIndex(InterfaceJogo controleJogo, boolean colisao) {
		if (colisao) {
			agressivo = false;
			return;
		}
	...
	private void tentarEscaparPilotoDaTraz(InterfaceJogo controleJogo,
		...
		if (calculaDiferencaParaAnterior < Util.intervalo(50 * controleJogo
			...
			boolean poupaMotor = motor > corrida;

			int combustivel = getCarro().porcentagemCombustivel();
			boolean econmizaCombustivel = combustivel > corrida;
			if (!controleJogo.isSemReabastacimento()) {
				econmizaCombustivel = true;
			}

			boolean maxUltimasVoltas = porcentagemCombustivel > 5
					&& controleJogo.verificaUltimasVoltas();
			boolean maxMotor = !superAquecido
	...
	private void memsagemTentaPasssar(InterfaceJogo controleJogo,
		...
		if (Math.random() < controleJogo.obterIndicativoCorridaCompleta()
			...
			case 1:
				txt = Lang.msg("048", new String[] { Html.bold(getNome()),
						Html.bold(carroPilotoDaFrente.getPiloto().getNome()) });
				controleJogo.info(Html.silver(txt));
				break;
			case 2:
				txt = Lang.msg("049", new String[] { Html.bold(getNome()),
						Html.bold(carroPilotoDaFrente.getPiloto().getNome()) });
				controleJogo.info(Html.silver(txt));
				break;
			case 3:
				txt = Lang.msg("050", new String[] { Html.bold(getNome()),
						Html.bold(carroPilotoDaFrente.getPiloto().getNome()) });
				controleJogo.info(Html.silver(txt));
				break;
			case 4:
				txt = Lang.msg("051", new String[] { Html.bold(getNome()),
						Html.bold(carroPilotoDaFrente.getPiloto().getNome()) });
				controleJogo.info(Html.silver(txt));
				break;
