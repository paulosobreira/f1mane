...
public class Piloto implements Serializable {
	...
	public double calculaGanhoMedio(double ganho) {
		if (aceleracao < 7) {
			if (carro.testePotencia())
				aceleracao *= 1.1;
			else {
				aceleracao *= 1.05;
			}
			ganho = ganho / (8 - aceleracao);
			listGanho.add(ganho);
			return ganho;
		} else {
			aceleracao = 7;
	...
	public boolean mudarTracado(int pos, InterfaceJogo interfaceJogo,
		...
		if (!verificaColisaoPos(interfaceJogo, pos)) {
			...
			if (pos == 4 || pos == 5 || getTracadoAntigo() == 4
					|| getTracadoAntigo() == 5) {
				mod *= 3;
			}
