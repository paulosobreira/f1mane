...
public class Piloto implements Serializable {
	...
	private void processaTurbulencia(InterfaceJogo controleJogo) {
		...
		if (diff < distLimiteTurbulencia
				&& !verificaForaPista(carroPilotoDaFrenteRetardatario
						.getPiloto())) {
			if (getTracado() != carroPilotoDaFrenteRetardatario.getPiloto()
					.getTracado()) {
				if (getNoAtual().verificaRetaOuLargada()) {
					multiplicadoGanhoTurbulencia += (getCarro().testePotencia() && getCarro()
							.testeAerodinamica()) ? 0.3 : 0.1;
				} else {
					multiplicadoGanhoTurbulencia += (testeHabilidadePilotoAerodinamicaFreios(controleJogo) ? 0.3
							: 0.1);
				}
			}
		...
		if (getPosicao() < 8
			...
			if (tentarEscaparAtras) {
				String txt = Lang.msg(
						...
						new String[] {
								Html.bold(getNome()),
								Html.bold(carroPilotoAtras.getPiloto()
										.getNome()) });
				controleJogo.info(Html.silver(txt));
			}
		}
	...
	private boolean tentarPassaPilotoDaFrente(InterfaceJogo controleJogo) {
		...
		int distBrigaMin = 0;
		if (calculaDiferencaParaProximo > distBrigaMin
				&& calculaDiferencaParaProximo < distBrigaMax
				&& testeHabilidadePiloto(controleJogo)) {
			modoIADefesaAtaque(controleJogo);
			return true;
		}
	...
	private void modoIADefesaAtaque(InterfaceJogo controleJogo) {
		...
		if (maxUltimasVoltas || maxCorrida
				|| calculaDiferencaParaAnterior < 300
