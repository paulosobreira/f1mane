...
public class Piloto implements Serializable {
		...
		if (diff < distLimiteTurbulencia && !verificaForaPista(carroPilotoDaFrenteRetardatario.getPiloto())) {
			if (getTracado() != carroPilotoDaFrenteRetardatario.getPiloto().getTracado()) {
				if (getNoAtual().verificaRetaOuLargada()) {
					...
					multiplicadoGanhoTurbulencia += (testeHabilidadePilotoAerodinamicaFreios(controleJogo) ? 0.3 : 0.1);
				}
			}
	...
	private void processaFreioNaReta(InterfaceJogo controleJogo) {
		...
		if (obterProxCurva != null && obterProxCurva.verificaCruvaBaixa()) {
			...
			if (val < distAfrente && getNoAtual().verificaRetaOuLargada()) {
				...
				double minMulti = 0.7;
				if (!controleJogo.isModoQualify() && (controleJogo.isChovendo())) {
					minMulti -= 0.3;
	...
	private void processaLimitadorGanho(InterfaceJogo controleJogo) {
		...
		if (evitaBaterCarroFrente) {
			ganho *= (calculaDiffParaProximoRetardatarioTracado / limiteEvitarBatrCarroFrente);
			return;
		}
	...
	private boolean verificaPodeUsarDRS(InterfaceJogo controleJogo) {
		if (controleJogo.isDrs() && getPtosBox() == 0 && getNumeroVolta() > 1 && getNoAtual().verificaRetaOuLargada()
	...
	private void processaIAnovoIndex(InterfaceJogo controleJogo) {
		...
		if (getPosicao() < 8 && Math.random() < controleJogo.obterIndicativoCorridaCompleta()
				&& msgTentativaNumVolta == getNumeroVolta()) {
			msgTentativaNumVolta = getNumeroVolta() + Util.intervalo(1, 3);
			if (tentaPassarFrete) {
			...
			if (tentarEscaparAtras) {
				String txt = Lang.msg("tentarEscaparAtras",
						new String[] { Html.bold(getNome()), Html.bold(carroPilotoAtras.getPiloto().getNome()) });
				controleJogo.info(Html.silver(txt));
			}
		}
	...
	private void processaMudarTracado(InterfaceJogo controleJogo) {
		...
		if (diff < diffAnt) {
			if (diff < 150) {
				if (testeHabilidadePiloto(controleJogo)) {
					...
					controleJogo.fazPilotoMudarTracado(this, carroPilotoDaFrenteRetardatario.getPiloto());
			...
			if (diffAnt < 200 && diffAnt > 50 && pilotoAtraz.getPtosBox() == 0 && testeHabilidadePiloto(controleJogo)
				...
				if (mudarTracado(pilotoAtraz.getTracado(), controleJogo) && noAtual.verificaRetaOuLargada()) {
					mudouTracadoReta++;
				}
		...
		if (evitaBaterCarroFrente && getTracado() == carroPilotoDaFrenteRetardatario.getPiloto().getTracado()) {
			int novapos = Util.intervalo(0, 2);
			int cont = 0;
			while (novapos == carroPilotoDaFrenteRetardatario.getPiloto().getTracado() && cont < 10) {
	...
	private boolean tentarPassaPilotoDaFrente(InterfaceJogo controleJogo) {
		...
		int distBrigaMin = 0;
		if (calculaDiferencaParaProximo > distBrigaMin && calculaDiferencaParaProximo < distBrigaMax
				&& testeHabilidadePiloto(controleJogo)) {
			modoIADefesaAtaque(controleJogo);
			return true;
		}
	...
	private void modoIADefesaAtaque(InterfaceJogo controleJogo) {
		...
		if (maxUltimasVoltas || maxCorrida || calculaDiferencaParaAnterior < 300
				|| calculaDiffParaProximoRetardatario < 300) {
			getCarro().setGiro(Carro.GIRO_MAX_VAL);
		}
		if (drsAtivado && !superAquecido && temMotor && porcentagemCombustivel > 5) {
			getCarro().setGiro(Carro.GIRO_MAX_VAL);
		}
		if (temMotor && !superAquecido && calculaDiffParaProximoRetardatario < 150 && porcentagemCombustivel > 5) {
			getCarro().setGiro(Carro.GIRO_MAX_VAL);
		}
	...
	private void verificaMudancaRegime(InterfaceJogo controleJogo) {
		...
		if (testeHabilidadePiloto(controleJogo)) {
			if (carro.verificaCarroNormal(controleJogo) && !qualyJogHumano) {
	...
	public boolean mudarTracado(int mudarTracado, InterfaceJogo interfaceJogo) {
		...
		if (!verificaColisaoAoMudarDeTracado(interfaceJogo, mudarTracado)) {
			...
			ultimaMudancaPos = System.currentTimeMillis() + (Constantes.CICLO * 10);
		}
		return false;
	...
	public int getCalculaDiferencaParaProximo() {
