...
public class Piloto implements Serializable, PilotoSuave {
	...
	private void processaIAnovoIndex(InterfaceJogo controleJogo) {
		...
		if (limiteGanho && controleJogo.verificaNivelJogo() && testeHabilidadePiloto(controleJogo)) {
			getCarro().setGiro(Carro.GIRO_NOR_VAL);
			setModoPilotagem(NORMAL);
		}
	...
	public boolean verificaNaoPrecisaDesviar(InterfaceJogo controleJogo, Piloto pilotoFrente) {
		...
		if (controleJogo.isSafetyCarNaPista()) {
			...
			if (pilotoFrente.isDesqualificado() || pilotoFrente.getCarro().isPaneSeca()
					|| Carro.ABANDONOU.equals(danificado) || Carro.BATEU_FORTE.equals(danificado)
	...
	public Rectangle2D centralizaDianteiraTrazeiraCarro(InterfaceJogo controleJogo) {
		if (getNoAnterior() != null && getDiateira() != null && getCentro() != null && getTrazeira() != null
				&& !emMovimento()) {
			return null;
		}
