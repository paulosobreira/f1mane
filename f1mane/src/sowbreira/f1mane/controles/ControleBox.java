		...
		if (!(cont > (circuito.getEntradaBoxIndex() - 50) && cont < (circuito
			...
			if (piloto.getPtosBox() == 0
				...
				int indexParada = piloto.obterPista(controleJogo)
						.indexOf(nobox);
				boolean mais90Porcent = (piloto.getNoAtual().getIndex() / controleJogo
						.getNosDoBox().size()) < 0.9;
	...
	private boolean verificaTemCarroPassandoSaida() {
		...
		for (Iterator iterator = pilotos.iterator(); iterator.hasNext();) {
			...
			if (saidaBox.getIndex() + 50 > noAtual.getIndex()
					&& noAtual.getIndex() > saidaBox.getIndex()
							- (controleCorrida.isSafetyCarNaPista() ? 300 : 200)) {
				return true;
			}
		}
		return false;
