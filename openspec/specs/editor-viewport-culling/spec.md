...
Defines viewport-based culling in the circuit editor's main object-drawing loop (`desenhaObjetosNivel`): skipping `desenha()` calls for `ObjetoPista` instances whose bounds don't intersect the visible viewport, as a pure performance optimization that must never change what is actually rendered when an object is drawn, and must never affect in-progress creation/edit overlays.
