...
Garante que todo artefato de código (pacote Java, config de build/servlet/JPA, atributo `class` embutido nos XMLs de circuito) referencia exclusivamente `br.flmane`/`flmane`, sem nenhuma ocorrência residual de `br.f1mane`/`f1mane` fora das exclusões de escopo registradas (properties de carro/piloto, nomes de circuito, `openspec/specs/`, `openspec/changes/archive/`, `CLAUDE.md`).
