# objeto-construcao-tipos

## Purpose

Permitir que `ObjetoConstrucao` represente diferentes formas de construção de cenário (`QUADRADO`, `REDONDO`, `CAMINHAO`, `BARCO`) através de uma propriedade `tipo`, com empilhamento (repetição deslocada da forma) tratado como uma característica transversal a qualquer tipo, controlada por quantidade, direção e grau de empilhamento independentes de `largura`/`altura`.
