...
`ObjetoEscapada#obterAreaVisual()` SHALL retornar o retângulo bruto dos vértices (`obterArea()`) expandido pela espessura (`largura`) com que o traçado é desenhado, e não apenas esse retângulo bruto. Essa área é usada para desenhar o contorno de destaque quando a escapada está selecionada na lista de objetos do circuito, garantindo que o destaque cubra o traçado realmente visível — não é usada para hit-testing de clique no canvas (ver requisito abaixo).
