# editor-copiar-colar-objetos

## Purpose

Permitir, no editor de circuitos, copiar um ou mais objetos selecionados na lista única e colá-los como cópias independentes ao final da coleção do `Circuito` correspondente ao seu tipo (`objetos` para objetos de função, `objetosCenario` para objetos de cenário), posicionados no ponto que o usuário clicar após um diálogo de aviso, preservando tamanho, ângulo, cores, pontos e demais propriedades do objeto original (exceto a posição), imediatamente visíveis sem exigir recarregar o circuito, e com a memória de objetos copiados sobrevivendo à troca de circuito na mesma instância do editor.
