# web-marcas-pneu-consumo-backend

## Purpose

Tornar o backend a fonte autoritativa das marcas de pneu exibidas no cliente web: `ControleJogosServer` expõe o snapshot completo das `TravadaRoda` acumuladas na pista no payload de posição, e o cliente HTML5 passa a acumular e desenhar as marcas a partir dessa lista por nó, em vez de inferi-las pelo status transitório (`"T"`/`"M"`) de cada piloto no tick corrente.
