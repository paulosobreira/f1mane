...
import br.flmane.servidor.entidades.Comandos;
...
public class ControleCampeonatoCliente {
	...
	public void criarCampeonato() throws Exception {
		...
		for (int i = 0; i < corridas.size(); i++) {
			...
			if (controlePaddockCliente != null) {
				corridaCampeonato.setLoginCriador(controlePaddockCliente.getSessaoCliente().getIdUsuario());
			}
			campeonato.getCorridaCampeonatos().add(corridaCampeonato);
		}
		if (controlePaddockCliente != null) {
			campeonato.setLoginCriador(controlePaddockCliente.getSessaoCliente().getIdUsuario());
			...
			public Object getValueAt(int rowIndex, int columnIndex) {
				...
				switch (columnIndex) {
					...
					case 1:
						return value[1];
					case 2:
						boolean val = ((Boolean) value[2]).booleanValue();
	...
	protected void carregaCampeonatoSelecionado(JTable corridasTable) {
		...
		if (ret == JOptionPane.YES_OPTION) {
			...
			if (controlePaddockCliente.retornoNaoValido(Integer.valueOf(ret))) {
				return;
			}
			if (retorno == null) {
	...
	public void geraListaPilotosPontos() {
		...
		for (CorridaCampeonatoSrv corridaCampeonato : campeonato.getCorridaCampeonatos()) {
			...
			for (Iterator iterator = pilotosPontos.iterator(); iterator.hasNext(); ) {
				...
				pilotosPontosCampeonato.setVitorias(computaVitorias(pilotosPontosCampeonato.getNome()).intValue());
			}
	...
	public void geraListaJogadoresPontos() {
		...
		for (CorridaCampeonatoSrv corridaCampeonato : campeonato.getCorridaCampeonatos()) {
			...
			for (Iterator iterator = jogadoresPontos.iterator(); iterator.hasNext(); ) {
				...
				pilotosPontosCampeonato.setVitorias(computaVitoriasJogador(Long.valueOf(pilotosPontosCampeonato.getId())).intValue());
			}
