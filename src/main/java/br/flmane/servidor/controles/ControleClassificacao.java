...
import br.flmane.servidor.entidades.persistencia.CorridasDadosSrv;
...
public class ControleClassificacao {
    ...
    public void processaCorrida(long tempoInicio, long tempoFim, Map mapVoltasJogadoresOnline, List pilotos,
        ...
        try {
            for (Iterator iter = pilotos.iterator(); iter.hasNext(); ) {
                ...
                if (piloto.isJogadorHumano() && !Util.isNullOrEmpty(piloto.getIdUsuario())) {
                    JogadorDadosSrv jogadorDadosSrv = controlePersistencia.carregaDadosJogador(piloto.getIdUsuario(),
                            session);
                    ...
                    CarreiraDadosSrv carreiraDadosSrv = controlePersistencia
                            .carregaCarreiraJogador(piloto.getIdUsuario(), false, session);
    ...
    public void processarPontos(Map mapVoltasJogadoresOnline, Piloto piloto, CorridasDadosSrv corridasDadosSrv) {
        ...
        for (Iterator iterMaster = mapVoltasJogadoresOnline.keySet().iterator(); iterMaster.hasNext(); ) {
            ...
            for (Iterator iter = voltas.iterator(); iter.hasNext(); ) {
                ...
                if (jogadorOnline.getJogador().equals(piloto.getIdUsuario())) {
                    voltasConcluidas++;
    ...
    public Object verCarreira(String idUsuario, Session session) {
        CarreiraDadosSrv carreiraDadosSrv = controlePersistencia.carregaCarreiraJogador(idUsuario, true, session);
        if (carreiraDadosSrv != null) {
    ...
    public Object atualizaCarreira(ClientPaddockPack clientPaddockPack) {
        ...
        return atualizaCarreira(clientPaddockPack.getSessaoCliente().getIdUsuario(), carreiraDadosSrv);
    }

    public Object atualizaCarreira(String idUsuario, CarreiraDadosSrv carreiraDados) {
        Session session = controlePersistencia.getSession();
        try {
