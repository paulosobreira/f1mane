...
import br.flmane.servidor.entidades.persistencia.*;
import br.flmane.servidor.util.HibernateUtil;
import br.nnpe.Dia;
...
public class ControlePersistencia {
    ...
    public void gravarDados(Session session, FlManeDados... flManeDados) throws Exception {
        Transaction transaction = session.beginTransaction();
        try {
            for (FlManeDados dado : flManeDados) {
                session.merge(dado);
            }
            transaction.commit();
        } catch (Exception e) {
