...
public class PainelMenuLocal {
    ...
    private RoundRectangle2D campeonatoRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D continuaCampeonatoRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D sobreRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D proxPistaRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);
    ...
    private RoundRectangle2D anteTemporadaRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D pistaRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D menosVoltasRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D maisVoltasRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);
    ...
    private RoundRectangle2D solRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D chuvaRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);
    ...
    private RoundRectangle2D kersRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);

    private RoundRectangle2D trocaPneusRect = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);
    ...
    private RoundRectangle2D anteriroMenuRct = new RoundRectangle2D.Double(0, 0, 1, 1, 0, 0);
    ...
    private void desenhaCircuitoCorridaCampeonato(Graphics2D g2d, int x, int y) {
        ...
        carragaMiniCircuito(nmCircuitoMRO);
        circuitoMini.desenhaMiniCircuito(g2d, x, y);
        g2d.setFont(fontOri);
    }

    private void carragaMiniCircuito(String nmCircuitoMRO) {
    ...
    private void desenhaNomeCircuito(Graphics2D g2d, int x, int y, int i, String circuito) {
        ...
        String circuitoNm = (i + 1) + " " + Util.substVogais(circuito);
        ...
        if (MENU.equals(MENU_NOVO_CAMPEONATO_PILOTOS)) {
            try {
                ...
                if (cirucitosCampeonato.size() >= 5) {
                    ...
                    JOptionPane.showMessageDialog(mainFrame, Lang.msg("min5CorridasCampeonato"), Lang.msg("campeonato"),
    ...
    private void selecionaPistaAnterior() {
        Object objectAnt = null;
        for (int i = pistasAleatorias.size() - 1; i > -1; i--) {
            Object object = (Object) pistasAleatorias.get(i);
    ...
    private void selecionaProximaPista() {
        Object objectAnt = null;
        for (Iterator iterator = pistasAleatorias.iterator(); iterator.hasNext(); ) {
            Object object = (Object) iterator.next();
            if (circuitoSelecionado != null && circuitoSelecionado.equals(objectAnt)) {
                circuitoSelecionado = (String) object;
                break;
    ...
    private void desenhaCombustivel(Graphics2D g2d, int x, int y) {
        if (!this.reabastecimento) {
            combustivelSelecionado = 100;
            return;
        }
        Font fontOri = g2d.getFont();
        g2d.setFont(new Font(fontOri.getName(), Font.BOLD, 28));
    ...
    private void desenhaImgTipo(Graphics2D g2d, int x, int y, int tam, BufferedImage img) {
        g2d.setColor(lightWhite);
        int deslX = (tam / 2) - (img.getWidth() / 2);
        g2d.fillRoundRect(x + deslX, y - 65, img.getWidth() + 10, img.getHeight() + 2, 0, 0);
        if (PainelCircuito.desenhaImagens)
    ...
    private void desenhaSeletorCircuito(Graphics2D g2d, int centerX, int centerY) {
        ...
        if (circuitoSelecionado == null) {
            pistasAleatorias = new ArrayList<String>(controleJogo.getCircuitos().keySet());
            Collections.shuffle(pistasAleatorias);
            circuitoSelecionado = pistasAleatorias.get(0);
        }
        String nmCircuitoMRO = (String) controleJogo.getCircuitos().get(circuitoSelecionado);
        ...
        carragaMiniCircuito(nmCircuitoMRO);
        circuitoMini.desenhaMiniCircuito(g2d, centerX, centerY);
