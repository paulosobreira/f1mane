...
public class Piloto implements Serializable, PilotoSuave {
    ...
    private void verificaIaIrBox(InterfaceJogo controleJogo) {
        ...
        if (boxPneus) {
            if (voltas.size() > 3) {
                ...
                if (voltaUltima.getTempoNumero() > voltaPenultima.getTempoNumero() && voltaUltima.getTempoNumero() > voltaAntiPenultima.getTempoNumero()) {
                    box = true;
                }
            }
    ...
    private int calculaModificador(InterfaceJogo controleJogo) {
        ...
        if (Carro.GIRO_MAX_VAL == getCarro().getGiro()) {
            comparador += getCarro().testePotencia() ? 0.2 : 0.1;
        }
        ...
        if (!noAtual.verificaRetaOuLargada()) {
            if (AGRESSIVO.equals(modoPilotagem)) {
                comparador += testeHabilidadePiloto() ? 0.2 : 0.1;
            }
