...
public class ControleCampeonatoServidor {
    ...
    public Object criarCampeonato(CampeonatoSrv campeonato, String idUsuario) {
        ...
        try {
            ...
            JogadorDadosSrv jogadorDadosSrv = controlePersistencia
                    .carregaDadosJogador(idUsuario, session);
    ...
    public void processaCorrida(long tempoInicio, long tempoFim,
        ...
        try {
            ...
            for (Iterator iter = pilotos.iterator(); iter.hasNext(); ) {
                ...
                JogadorDadosSrv jogadorDadosSrv = controlePersistencia
                        .carregaDadosJogador(piloto.getIdUsuario(), session);
                if (jogadorDadosSrv != null) {
    ...
    public CampeonatoSrv obterCampeonatoEmAberto(String idUsuario) {
        List pesquisaCampeonatos;
        Session session = controlePersistencia.getSession();
        try {
            pesquisaCampeonatos = controlePersistencia
                    .pesquisaCampeonatosEmAberto(idUsuario, session, true);
        } finally {
            if (session.isOpen()) {
                session.close();
            }
    ...
    public CampeonatoTO obterCampeonatoEmAbertoTO(String idUsuario) {
        CampeonatoSrv campeonato = obterCampeonatoEmAberto(idUsuario);
        if (campeonato == null) {
            return null;
        }
        CampeonatoTO campeonatoTO = new CampeonatoTO();
